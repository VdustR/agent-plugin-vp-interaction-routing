...
const INVARIANTS = [
  ...
  [SKILL, /End task-owned Computer Use, browser, and desktop-automation sessions after\s+success, failure, cancellation, or handoff/s,
    "skill must end task-owned interaction sessions on every exit path"],
  [SKILL, /Read the interface or process state after cleanup and confirm that the session\s+ended/s,
    "skill must verify Computer Use session cleanup"],
  [SKILL, /Leave the user's existing apps, tabs, windows, documents,\s+and shared services open/s,
    "skill cleanup must preserve existing user state"],
  [SKILL, /A resource that existed before the task remains user-owned when\s+the task uses or activates it/s,
    "skill cleanup must not claim lifecycle ownership of existing user resources"],
  ...
  [NATIVE, /session that existed before the task remains user-owned.*success, failure, cancellation, or\s+handoff.*confirm that the session ended/s,
    "native UI guidance must close and verify task-owned sessions on every exit path"],
  ...
  [BRIDGE_REFERENCE, /A bare `health` proves compatibility, not that the service will serve.*none of them\s+performs a Computer Use action/s,
    "the bridge reference must record what a bare health verdict does not prove"],
  [BRIDGE_REFERENCE, /Pass `probe_app` to close that gap.*verdict reflects the answer/s,
    "the bridge reference must name the check that proves the service is serving"],
  [BRIDGE_REFERENCE, /quotes the upstream reason rather than diagnosing a cause/s,
    "the probe failure verdict must not be documented as diagnosing a cause"],
