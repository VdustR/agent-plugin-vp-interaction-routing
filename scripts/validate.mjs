...
const INVARIANTS = [
  ...
  [BROWSER, /Pane displayed, tab selected[\s\S]*Pane displayed, tab never selected[\s\S]*Pane hidden \| Not reproduced/,
    "browser guidance must record which pane conditions were measured"],
  [BROWSER, /`document\.hasFocus\(\)` was `false` in every measured condition/,
    "focus-gated page logic must stay routed to a shim or a real browser"],
  [BROWSER, /`navigate` displays the pane, so a page cannot be loaded into a hidden one/,
    "browser guidance must record that a hidden pane cannot be navigated into"],
  ...
  [BROWSER, /only when the measured rAF\s+count is zero.*Take one\s+`computer screenshot`.*lazy-state predicate.*escalate to Tier C/s,
    "the single-frame render pump must be gated on a measured stalled rAF"],
